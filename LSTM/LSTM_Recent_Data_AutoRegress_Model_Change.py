...
Prepares data for a model that outputs for all df_cols not just TMAX.
Also, for X_test - this is only a tensor of shape (1, seq_length, num_input).
...
def prepare_data(df_cols=["TMAX"], seq_length=30):
    ...
    for i in range(len(df_combined_scaled) - seq_length):
        ...
        targets.append(df_combined_scaled[i + seq_length]) 
...
config = {
        ...
        "num_epochs": 200,
        "learning_rate": 0.001, 
        "experiment_name": "LSTM_Autogress_Model_Changed",
        "num_output" : 12
    }
...
os.makedirs("images/LSTM_Autogress_Model_Changed", exist_ok=True)

# write results to csv
csv_file = "experiment_results/LSTM/LSTM_Autogress_Model_Changed.csv"
